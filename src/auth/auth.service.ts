...
export class AuthService {
  ...
  async signinpLocal(dto: AuthDto): Promise<Tokens> {
    ...
    if (!user) {
      throw new ForbiddenException('Access Denied: There is no user');
    }

    const passwordMatches = await bcrypt.compare(dto.password, user.hash);
    if (!passwordMatches) {
      throw new ForbiddenException('Access Denied: Password is not correct');
    }
  ...
  async refreshTokens(userId: number, rt: string) {
    ...
    if (!rtMatcher) {
      throw new ForbiddenException('Acces Denier');
    }
